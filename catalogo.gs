...
// El browser del usuario nunca hace una request a Drive.
// ---------------------------------------------------------------
...
function buildCard(p, sheetName) {
  ...
  // Si hay URL directa (formula IMAGE), úsala; sino carga lazy desde el sheet
  var imgHtml;
  if (p.imgUrl) {
...
function getHtml(vigencia, navLinks, sectionsHtml, bannerUrl, logoDataUrl, sheetOrderJson) {
  // Logo: imagen real si está disponible, SVG de fallback si no
  ...
  // Script de carga lazy de imágenes desde el sheet vía google.script.run
